...
const CIAATOPUP_API_KEY = "CiaaTopUp_7wspuwx0lwdmslkp";
const CIAATOPUP_BASE_URL = "https://ciaatopup.my.id/h2h";
...
const mongoURI = 'mongodb+srv://aqila:diboystrongboys@cluster0.uita3mr.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
...
const ciaatopupAPI = axios.create({
    baseURL: CIAATOPUP_BASE_URL,
    headers: { 'X-APIKEY': CIAATOPUP_API_KEY }
});

app.get('/api/layanan', async (req, res) => {
    try {
        console.log('[LOG] Meminta daftar layanan dari CiaaTopUp...');
        const response = await ciaatopupAPI.get('/layanan/price-list');
...
app.post('/api/buat-transaksi', async (req, res) => {
    ...
    try {
        ...
        const depositResponse = await ciaatopupAPI.get(`/deposit/create?nominal=${price}`);
        console.log('[LOG] Respon dari CiaaTopUp (Buat Deposit):', JSON.stringify(depositResponse.data, null, 2));
        ...
        if (depositResponse.data && depositResponse.data.success) {
            ...
            transactions[internalTrxId] = {
                CiaaTopUpDepositId: depositData.id,
                productCode: code,
                target: tujuan,
                price: price,
                status: 'menunggu_pembayaran',
                CiaaTopUpOrderId: null
            };
...
app.get('/api/cek-status-deposit', async (req, res) => {
    ...
    if (transaction.status !== 'menunggu_pembayaran') {
        return res.json({ depositStatus: 'success', orderId: transaction.CiaaTopUpOrderId });
    }
    
    try {
        console.log(`[LOG] Mengecek status deposit CiaaTopUp ID: ${transaction.CiaaTopUpDepositId}`);
        const statusResponse = await CiaaTopUpAPI.get(`/deposit/status?id=${transaction.CiaaTopUpDepositId}`);
        console.log('[LOG] Respon dari CiaaTopUp (Cek Deposit):', JSON.stringify(statusResponse.data, null, 2));
        ...
        if (statusResponse.data.success && statusResponse.data.data.status === 'success') {
            ...
            const orderResponse = await CiaaTopUpAPI.get(`/order/create?code=${transaction.productCode}&tujuan=${transaction.target}`);
            console.log('[LOG] Respon dari CiaaTopUp (Buat Order):', JSON.stringify(orderResponse.data, null, 2));

            if (orderResponse.data && orderResponse.data.success) {
                transaction.CiaaTopUpOrderId = orderResponse.data.data.id;
                transaction.status = 'menunggu_hasil_order';
                res.json({
                    depositStatus: 'success',
                    orderId: transaction.CiaaTopUpOrderId
                });
            } else {
...
app.get('/api/cek-status-order', async (req, res) => {
    ...
    try {
        console.log(`[LOG] Mengecek status order CiaaTopUp ID: ${orderId}`);
        const statusResponse = await CiaaTopUpAPI.get(`/order/check?id=${orderId}`);
        console.log('[LOG] Respon dari CiaaTopUp (Cek Order):', JSON.stringify(statusResponse.data, null, 2));
